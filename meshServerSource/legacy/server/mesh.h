...
public:
	...
	struct	Triangle
	{
        ...
        struct Triangle_uint32          //RK 12/21/09: OpenGL supports only 32-bit IDs.
        {                               //We have to avoid vtkIdType as much as possible! :(
                unsigned int v[3];
        };
...
public:
	...
	Vertex		*pVertices;
        Triangle_uint32	*pTriangles;    //RK 12/21/09: Changed from Triangle to Triangle_uint32
...
private:
	...
	int		FindPointInArray(CDoubleArray *pArray, double *pXYZ);
	void	ResetColor(CColor &cColor);

	/// Mix colors with specified ratio
	/// @param fMixFactor "weight" of source #1 colors [0..1]
	template<class T1, class T2, class T3>
		void	MixColors(T1 *pDst, const T2 *pSrc1, const T3 *pSrc2, double fMixFactor, int nComponents);
	...
	void	FindBounds(double fBounds[6]);
