...
public:
	...
	double	GetFloat(const wxString &strName, double fDefaultValue);
	wxString GetString(const wxString &strName, const wxString &strDefault);
	CDoubleArray *GetArray(const wxString &strName);

	/// @return false if no variable with name strName found
	/// @remarks if string contains doubleing-point value, this value is rounded to nearest int
	bool	GetInt(const wxString &strName, int *pValue);
	/// @return false if no variable with name strName found
	bool	GetFloat(const wxString &strName, double *pValue);
	/// @return false if no variable with name strName found
...
//	int	GetBinary(const wxString &strName, char *pBuf, int iLen);
	...
	void	SetFloat(const wxString &strName, double fValue);
	void	SetArray(const wxString &strName, CDoubleArray *pArray);
...
private:
	...
	WX_DECLARE_STRING_HASH_MAP(CDoubleArray*, HashMapArrayParams);
	...
	void	FormatArraysMapBinary(char **pResult, int *pLength);
        ...
        int     MemCpy_CondConv(char *dest, wxString &src);
