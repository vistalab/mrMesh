...
vtkCxxRevisionMacro(vtkPolyDataSingleSourceShortestPath, "$Revision: 1.2 $");
...
double vtkPolyDataSingleSourceShortestPath::EdgeCost(vtkPolyData *pd, vtkIdType u, vtkIdType v)
{
	...
	// 2005.07.18 RFD: added explicit int cast to all GetPoint calls 
	pd->GetPoint((int)u,p1);
	double p2[3];
	pd->GetPoint((int)v,p2);
	...
	if (this->UseScalarWeights)
	{
		...
		vtkDoubleArray *scalars = (vtkDoubleArray*)pd->GetPointData()->GetScalars();
		//double s1 = scalars->GetValue(u);
		double s2 = scalars->GetValue(v);
...
//#ifdef _WIN32
void* vtkPolyDataSingleSourceShortestPath::operator new(size_t nSize)
{
	void* p=malloc(nSize);
	return p;
}

void vtkPolyDataSingleSourceShortestPath::operator delete( void *p )
{
	free(p);
}
//#endif
