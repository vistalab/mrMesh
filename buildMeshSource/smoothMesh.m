function newMsh = smoothMesh(msh,QueryFlag)
%Smooth a VTK mesh
%
%   newMsh = smoothMesh(msh,QueryFlag)
%
...
%   mod_depth          (default 0.15)
%
% If you set the QueryFlag = 1, you will be asked about these parameter
% values.  
%
% Example:
...
%  msh = smoothMesh(msh);
%  msh = visualizeMesh(msh);
%
% Author GB


if ieNotDefined('msh'), error('This function needs a mesh input'); end
if ieNotDefined('QueryFlag'), QueryFlag = 0; end
...
if QueryFlag
    ...
    if(~isempty(resp))
        ...
        msh = meshSet(msh,'mod_depth',str2num(resp{4}));
    else
        error('Canceled by the user')
    end
end

fprintf('Smoothing mesh...')
